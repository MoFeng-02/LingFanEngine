...
await ExtendAsync("追加文本");  // 不换行追加
await WaitClickAsync();             // 等待点击
```
...
await WaitSkipableAsync(2.0);  // 可跳过的等待
```

### 菜单与输入

```csharp
var choice = await ChoiceAsync("你要怎么做？", "战斗", "逃跑");
if (choice == 0) { /* 战斗 */ }
...
```csharp
var result = await Ctrl.CallScreenAsync("save_panel");
...
```csharp
await Ctrl.PlayCutsceneAsync("Video/intro.mp4");
```
