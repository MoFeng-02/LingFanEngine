// ============================================================
// 第二章 - 迷雾森林
// 展示：战斗(random/if)、场景导航、过渡动画、SE 音效
// ============================================================
...
scene "forest_entry"
  ...
  button "深入森林" x=50% y=42% width=200 height=44 color="#88CCFF" nav="forest_deep" halign=center
  button "返回小镇" x=50% y=72% width=160 height=40 color="#FF8888" nav="title_main" halign=center
...
label forest_deep:
  ...
  shake intensity=15 duration=0.6
...
label wolf_battle:
  ...
  if {_local_wolf_hp <= 0}
    ...
    say "野狼扑上来咬了你，造成 {_local_wolf_dmg} 点伤害！" speaker="系统"
    set "player.hp" {player.hp - _local_wolf_dmg}
    if {player.hp <= 0}
      set "player.hp" 1
      say "你差点倒下，但勉强站住了。" speaker="旁白"
    say "野狼还在，HP 剩余 {_local_wolf_hp}。再挥一剑——" speaker="旁白"
    set "_local_dmg" {random(20, 40)}
    set "_local_wolf_hp" {_local_wolf_hp - _local_dmg}
    if {_local_wolf_hp <= 0}
      say "野狼终于倒下！+30 经验，+25 金币" speaker="系统"
      set "player.exp" {player.exp + 30}
      set "player.gold" {player.gold + 25}
      set "story.wolf_defeated" true
      navigate "cavern_entry"
    else
      say "野狼逃走了。" speaker="旁白"
