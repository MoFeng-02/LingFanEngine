...
/// <para>所有属性均为 key=value 形式，无元组语法糖。</para>
/// <para>位置用 x=/y=，尺寸用 width=/height=，字体大小用 size=。</para>
/// </summary>
public static class DslParser
{
    ...
    /// <para>顺序很重要：Try 用于可能部分匹配的分支</para>
    /// </summary>
    ...
    /// <para>所有属性均为 key=value 形式，可任意顺序。</para>
    /// </summary>
    ...
    /// </summary>
    /// <param name="line">DSL 元素行，如 `text "Hello" x=50% y=15% size=24 color="#FF0000"`</param>
    /// <returns>解析成功返回 UIElementEntity，失败返回 null</returns>
    ...
    /// </summary>
    /// <param name="line">DSL 行，如 `scene "title_main" layout=canvas type=menu`</param>
    /// <returns>解析成功返回 SceneHeader，失败返回 null</returns>
    ...
    /// <para>value 可为引号字符串、数字、布尔、颜色或裸标识符</para>
    /// <para>注意：Pidgin 的 Token+AtLeastOnceString 是贪婪的，会吞掉 " once" 后缀。</para>
    /// <para>因此这里只解析到行尾，在 C# 代码中剥离 " once" 后缀。</para>
    /// </summary>
    ...
    /// 解析 define 行
    /// </summary>
    /// <param name="line">DSL 行，如 `define "bg_volume" 0.8 once`</param>
    /// <returns>解析成功返回 DefineEntry，失败返回 null</returns>
    ...
    /// 解析 label 行
    /// </summary>
    /// <param name="line">DSL 行，如 `label intro:`</param>
    /// <returns>解析成功返回 label 名，失败返回 null</returns>
    ...
    private static UIElementEntity BuildEntity(
        ...
        System.Collections.Generic.IEnumerable<PropEntry> props)
    {
